...
class CCPert_sp(object):
    def __init__(self, name, pert, ccsd, cclambda, omega):
        ...
        self.t1 = ccsd.t1
        self.t2 = ccsd.t2
        self.Fsp = np.float32(ccsd.F)
        self.t1_sp = np.float32(ccsd.t1)
        self.t2_sp = np.float32(ccsd.t2)
        ...
        self.l1 = cclambda.l1
        self.l2 = cclambda.l2
        self.l1_sp = np.float32(cclambda.l1)
        self.l2_sp = np.float32(cclambda.l2)
        self.omega = omega
    ...
    def inhomogenous_y2(self):
        ...
        r_y2 = ndot('ia,jb->ijab', self.l1_sp, self.build_Aov(), prefactor=2.0)
        r_y2 -= ndot('ja,ib->ijab', self.l1_sp, self.build_Aov())
    ...
    def inhomogenous_y1(self):
        ...
        # <O|A_bar|phi^a_i>
        r_y1 = np.float32(2.0) * self.build_Aov().copy()
        # <O|L1(0)|A_bar|phi^a_i>
        r_y1 -= ndot('im,ma->ia', self.build_Aoo(), self.l1_sp)
        r_y1 += ndot('ie,ea->ia', self.l1_sp, self.build_Avv())
        # <O|L2(0)|A_bar|phi^a_i>
        r_y1 += ndot('imfe,feam->ia', self.l2_sp, self.build_Avvvo())
