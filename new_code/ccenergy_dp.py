...
# F is time-dependent during the propagation*
    ...
    def build_Fae(self, F, t1, t2):
        ...
        Fae += ndot('mf,mafe->ae', t1, self.get_MO('ovvv'), prefactor=2.0)
        Fae += ndot('mf,maef->ae', t1, self.get_MO('ovvv'), prefactor=-1.0)
        Fae -= ndot('mnaf,mnef->ae', self.build_tilde_tau(t1, t2), self.get_MO('oovv'), prefactor=2.0)
        Fae -= ndot('mnaf,mnfe->ae', self.build_tilde_tau(t1, t2), self.get_MO('oovv'), prefactor=-1.0)
        return Fae
    ...
    def build_Fmi(self, F, t1, t2):
        ...
        Fmi += ndot('ne,mnie->mi', t1, self.get_MO('ooov'), prefactor=2.0)
        Fmi += ndot('ne,mnei->mi', t1, self.get_MO('oovo'), prefactor=-1.0)
        Fmi += ndot('inef,mnef->mi', self.build_tilde_tau(t1, t2), self.get_MO('oovv'), prefactor=2.0)
        Fmi += ndot('inef,mnfe->mi', self.build_tilde_tau(t1, t2), self.get_MO('oovv'), prefactor=-1.0)
        return Fmi
    ...
    def build_Fme(self, F, t1):
        ...
        Fme += ndot('nf,mnef->me', t1, self.get_MO('oovv'), prefactor=2.0)
        Fme += ndot('nf,mnfe->me', t1, self.get_MO('oovv'), prefactor=-1.0)
        return Fme

    # Build Eqn 6:
    def build_Wmnij(self, t1, t2):
        Wmnij = self.get_MO('oooo').copy()
        Wmnij += ndot('je,mnie->mnij', t1, self.get_MO('ooov'))
        Wmnij += ndot('ie,mnej->mnij', t1, self.get_MO('oovo'))
        ...
        Wmnij += ndot('ijef,mnef->mnij', self.build_tau(t1, t2), self.get_MO('oovv'), prefactor=1.0)
        return Wmnij

    # Build Eqn 8:
    def build_Wmbej(self, t1, t2):
        Wmbej = self.get_MO('ovvo').copy()
        Wmbej += ndot('jf,mbef->mbej', t1, self.get_MO('ovvv'))
        Wmbej -= ndot('nb,mnej->mbej', t1, self.get_MO('oovo'))
        tmp = (0.5 * t2)
        tmp += np.einsum('jf,nb->jnfb', t1, t1)
        Wmbej -= ndot('jnfb,mnef->mbej', tmp, self.get_MO('oovv'))
        Wmbej += ndot('njfb,mnef->mbej', t2, self.get_MO('oovv'), prefactor=1.0)
        Wmbej += ndot('njfb,mnfe->mbej', t2, self.get_MO('oovv'), prefactor=-0.5)
        return Wmbej
    ...
    def build_Wmbje(self, t1, t2):
        ...
        Wmbje -= ndot('jf,mbfe->mbje', t1, self.get_MO('ovvv'))
        Wmbje += ndot('nb,mnje->mbje', t1, self.get_MO('ooov'))
        tmp = (0.5 * t2)
        tmp += np.einsum('jf,nb->jnfb', t1, t1)
    ...
    def build_Zmbij(self, t1, t2):
        Zmbij = 0
        Zmbij += ndot('mbef,ijef->mbij', self.get_MO('ovvv'), self.build_tau(t1, t2))
        return Zmbij
    ...
    def r_T1(self, F, t1, t2):
        ...
        r_T1 += ndot('nf,nafi->ia', t1, self.get_MO('ovvo'), prefactor=2.0)
        r_T1 += ndot('nf,naif->ia', t1, self.get_MO('ovov'), prefactor=-1.0)
        r_T1 += ndot('mief,maef->ia', t2, self.get_MO('ovvv'), prefactor=2.0)
        r_T1 += ndot('mife,maef->ia', t2, self.get_MO('ovvv'), prefactor=-1.0)
        r_T1 -= ndot('mnae,nmei->ia', t2, self.get_MO('oovo'), prefactor=2.0)
        r_T1 -= ndot('mnae,nmie->ia', t2, self.get_MO('ooov'), prefactor=-1.0)
    ...
    def r_T2(self, F, t1, t2):
        ...
        tmp = ndot('ie,abej->ijab', t1, self.get_MO('vvvo'), prefactor=1.0)
        r_T2 += tmp
        r_T2 += tmp.swapaxes(0, 1).swapaxes(2, 3)

        # P(ab) {-t_ma <mb||ij>} -> P^(ab)_(ij) {-t_ma <mb|ij>}
        tmp = ndot('ma,mbij->ijab', t1, self.get_MO('ovoo'), prefactor=1.0)
        r_T2 -= tmp
        r_T2 -= tmp.swapaxes(0, 1).swapaxes(2, 3)
    ...
    def update(self):
        ...
        r1 = self.r_T1(self.F, self.t1, self.t2)
        r2 = self.r_T2(self.F, self.t1, self.t2)
    ...
    def compute_energy(self,
        ...
        for CCSD_iter in range(1, maxiter + 1):
            ...
            CCSD_iter, np.real(CCSDcorr_E), np.real(CCSDcorr_E - CCSDcorr_E_old), diis_object.diis_size))
