...
class ccLambda(object):
    ...
    # dp for ccpert
    def build_Loovv_dp(self):
        tmp = self.get_MO('oovv').copy()
        Loovv = 2.0 * tmp - tmp.swapaxes(2, 3)
        return Loovv

    def build_Looov_dp(self):
        tmp = self.get_MO('ooov').copy()
        Looov = 2.0 * tmp - tmp.swapaxes(0, 1)
        return Looov

    def build_Lvovv_dp(self):
    ...
    def r_l1(self, F, t1, t2, l1, l2):
        r_l1 = 2.0 * np.float64(self.build_Hov(F, t1).copy())
        r_l1 += np.float64(ndot('ie,ea->ia', l1, self.build_Hvv(F, t1, t2)))
        r_l1 -= np.float64(ndot('im,ma->ia', self.build_Hoo(F, t1, t2), l1))
        tmp = ndot('ieam,me->ia', self.build_Hovvo(t1, t2), l1, prefactor=2.0)
        tmp += ndot('iema,me->ia', self.build_Hovov(t1, t2), l1, prefactor=-1.0)
        r_l1 += np.float64(tmp)
        r_l1 += np.float64(ndot('imef,efam->ia', l2, self.build_Hvvvo(F, t1, t2)))
        r_l1 -= np.float64(ndot('iemn,mnae->ia', self.build_Hovoo(F, t1, t2), l2))
        tmp = ndot('eifa,ef->ia', self.build_Hvovv(t1), self.build_Gvv(t2, l2), prefactor=2.0)
        tmp += ndot('eiaf,ef->ia', self.build_Hvovv(t1), self.build_Gvv(t2, l2), prefactor=-1.0)
        r_l1 -= np.float64(tmp)
        tmp = ndot('mina,mn->ia', self.build_Hooov(t1), self.build_Goo(t2, l2), prefactor=2.0)
        tmp += ndot('imna,mn->ia', self.build_Hooov(t1), self.build_Goo(t2, l2), prefactor=-1.0)
        r_l1 -= np.float64(tmp)

        return r_l1

    def r_l2(self, F, t1, t2, l1, l2):
        r_l2 = np.float64(self.build_Loovv().copy())
        tmp = ndot('ia,jb->ijab', l1, self.build_Hov(F, t1), prefactor=2.0)
        tmp -= ndot('ja,ib->ijab', l1, self.build_Hov(F, t1))
        r_l2 += np.float64(tmp)
        r_l2 += np.float64(ndot('ijeb,ea->ijab', l2, self.build_Hvv(F, t1, t2)))
        r_l2 -= np.float64(ndot('im,mjab->ijab', self.build_Hoo(F, t1, t2), l2))
        r_l2 += np.float64(ndot('ijmn,mnab->ijab', self.build_Hoooo(t1, t2), l2, prefactor=0.5))
        r_l2 += np.float64(ndot('ijef,efab->ijab', l2, self.build_Hvvvv(t1, t2), prefactor=0.5))
        tmp = ndot('ie,ejab->ijab', l1, self.build_Hvovv(t1), prefactor=2.0)
        tmp += ndot('ie,ejba->ijab', l1, self.build_Hvovv(t1), prefactor=-1.0)
        r_l2 += np.float64(tmp)
        tmp = ndot('mb,jima->ijab', l1, self.build_Hooov(t1), prefactor=2.0)
        tmp += ndot('mb,ijma->ijab', l1, self.build_Hooov(t1), prefactor=-1.0)
        r_l2 -= np.float64(tmp)
        tmp = ndot('ieam,mjeb->ijab', self.build_Hovvo(t1, t2), l2, prefactor=2.0)
        tmp += ndot('iema,mjeb->ijab', self.build_Hovov(t1, t2), l2, prefactor=-1.0)
        r_l2 += np.float64(tmp)
        r_l2 -= np.float64(ndot('mibe,jema->ijab', l2, self.build_Hovov(t1, t2)))
        r_l2 -= np.float64(ndot('mieb,jeam->ijab', l2, self.build_Hovvo(t1, t2)))
        r_l2 += np.float64(ndot('ijeb,ae->ijab', self.build_Loovv(), self.build_Gvv(t2, l2)))
        r_l2 -= np.float64(ndot('mi,mjab->ijab', self.build_Goo(t2, l2), self.build_Loovv()))
