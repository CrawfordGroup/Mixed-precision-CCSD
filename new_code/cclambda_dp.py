...
class ccLambda_dp(object):
    ...
    def build_Hoooo(self, t1, t2):
        ...
        Hoooo += ndot('je,mnie->mnij', t1, self.get_MO('ooov'))
        Hoooo += ndot('ie,mnej->mnij', t1, self.get_MO('oovo'))
        Hoooo += ndot('ijef,mnef->mnij', self.build_tau(t1, t2), self.get_MO('oovv'))
        return Hoooo
    ...
    def build_Hvvvv(self, t1, t2):
        ...
        Hvvvv -= ndot('mb,amef->abef', t1, self.get_MO('vovv'))
        Hvvvv -= ndot('ma,bmfe->abef', t1, self.get_MO('vovv'))
        Hvvvv += ndot('mnab,mnef->abef', self.build_tau(t1, t2), self.get_MO('oovv'))
        return Hvvvv

    def build_Hvovv(self, t1):
        """ <am|Hbar|ef> = <am||ef> - t_na <nm||ef> """
        Hvovv = self.get_MO('vovv').copy()
        Hvovv -= ndot('na,nmef->amef', t1, self.get_MO('oovv'))
        return Hvovv

    def build_Hooov(self, t1):
        """ <mn|Hbar|ie> = <mn||ie> + t_if <mn||fe> """
        Hooov = self.get_MO('ooov').copy()
        Hooov += ndot('if,mnfe->mnie', t1, self.get_MO('oovv'))
        return Hooov
    ...
    def build_Hovvo(self, t1, t2):
        ...
        Hovvo += ndot('jf,mbef->mbej', t1, self.get_MO('ovvv'))
        Hovvo -= ndot('nb,mnej->mbej', t1, self.get_MO('oovo'))
        Hovvo -= ndot('jnfb,nmfe->mbej', self.build_tau(t1, t2), self.get_MO('oovv'))
        Loovv = self.build_Loovv()
    ...
    def build_Hovov(self, t1, t2):
        ...
        Hovov += ndot('jf,bmef->mbje', t1, self.get_MO('vovv'))
        Hovov -= ndot('nb,mnje->mbje', t1, self.get_MO('ooov'))
        Hovov -= ndot('jnfb,nmef->mbje', self.build_tau(t1, t2), self.get_MO('oovv'))
        return Hovov
    ...
    def build_Hvvvo(self, F, t1, t2):
        ...
        Hvvvo += ndot('if,abef->abei', t1, self.get_MO('vvvv'))
        tmp = ndot('if,ma->imfa', t1, t1)
        Hvvvo -= ndot('imfa,mbef->abei', tmp, self.get_MO('ovvv'))
        Hvvvo -= ndot('imfb,amef->abei', tmp, self.get_MO('vovv'))
        tmp = ndot('mnef,if->mnei', self.get_MO('oovv'), t1)
        Hvvvo += ndot('mnab,mnei->abei', t2, tmp)
        tmp = ndot('if,ma->imfa', t1, t1)
        tmp1 = ndot('mnef,nb->mbef', self.get_MO('oovv'), t1)
        Hvvvo += ndot('imfa,mbef->abei', tmp, tmp1)

        # 0.5 * tau_mnab <mn||ei>

        Hvvvo += ndot('mnab,mnei->abei', self.build_tau(t1, t2), self.get_MO('oovo'))

        # - P(ab) t_miaf <mb||ef>

        Hvvvo -= ndot('imfa,mbef->abei', t2, self.get_MO('ovvv'))
        Hvvvo -= ndot('imfb,amef->abei', t2, self.get_MO('vovv'))
        Lvovv = self.build_Lvovv()
        Hvvvo += ndot('mifb,amef->abei', t2, Lvovv)

        # - P(ab) t_ma <mb||ei>

        Hvvvo -= ndot('mb,amei->abei', t1, self.get_MO('vovo'))
        Hvvvo -= ndot('ma,bmie->abei', t1, self.get_MO('voov'))

        # P(ab) t_ma * t_nibf <mn||ef>

        tmp = ndot('mnef,ma->anef', self.get_MO('oovv'), t1)
        Hvvvo += ndot('infb,anef->abei', t2, tmp)
        Loovv = self.build_Loovv()
        tmp = ndot('mnef,ma->nafe', Loovv, t1)
        Hvvvo -= ndot('nifb,nafe->abei', t2, tmp)
        tmp = ndot('nmef,mb->nefb', self.get_MO('oovv'), t1)
    ...
    def build_Hovoo(self, F, t1, t2):
        ...
        Hovoo -= ndot('nb,mnij->mbij', t1, self.get_MO('oooo'))
        tmp = ndot('ie,nb->ineb', t1, t1)
        Hovoo -= ndot('ineb,mnej->mbij', tmp, self.get_MO('oovo'))
        Hovoo -= ndot('jneb,mnie->mbij', tmp, self.get_MO('ooov'))
        tmp = ndot('nb,mnef->mefb', t1, self.get_MO('oovv'))
        Hovoo -= ndot('ijef,mefb->mbij', t2, tmp)
        tmp = ndot('ie,jf->ijef', t1, t1)
        tmp1 = ndot('nb,mnef->mbef', t1, self.get_MO('oovv'))
        ...
        Hovoo -= ndot('ineb,mnej->mbij', t2, self.get_MO('oovo'))
        Hovoo -= ndot('jneb,mnie->mbij', t2, self.get_MO('ooov'))
        Looov = self.build_Looov()
        Hovoo += ndot('jnbe,mnie->mbij', t2, Looov)

        # P(ij) t_ie <mb||ej>

        Hovoo += ndot('je,mbie->mbij', t1, self.get_MO('ovov'))
        Hovoo += ndot('ie,mbej->mbij', t1, self.get_MO('ovvo'))

        # - P(ij) t_ie * t_njbf <mn||ef>

        tmp = ndot('ie,mnef->mnif', t1, self.get_MO('oovv'))
        Hovoo -= ndot('jnfb,mnif->mbij', t2, tmp)
        tmp = ndot('mnef,njfb->mejb', Loovv, t2)
        Hovoo += ndot('mejb,ie->mbij', tmp, t1)
        tmp = ndot('je,mnfe->mnfj', t1, self.get_MO('oovv'))
    ...
    def update_l(self):
        ...
        rl1 = self.r_l1(self.F, self.t1, self.t2, self.l1, self.l2)
        rl2 = self.r_l2(self.F, self.t1, self.t2, self.l1, self.l2)
