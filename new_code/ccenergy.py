...
# Modify the intermediates to single-precision and the t-amp eqns to mixed-precision
    # Build Eqn 3:
    def build_Fae(self, Fsp, t1_sp, ttau):
        Fae = self.get_F(Fsp, 'vv').copy()
        Fae -= ndot('me,ma->ae', self.get_F(Fsp, 'ov'), t1_sp, prefactor=0.5)
        Fae += ndot('mf,mafe->ae', t1_sp, self.get_MO_sp('ovvv'), prefactor=2.0)
        Fae += ndot('mf,maef->ae', t1_sp, self.get_MO_sp('ovvv'), prefactor=-1.0)
        Fae -= ndot('mnaf,mnef->ae', np.float32(ttau), self.get_MO_sp('oovv'), prefactor=2.0)
        Fae -= ndot('mnaf,mnfe->ae', np.float32(ttau), self.get_MO_sp('oovv'), prefactor=-1.0)
        return Fae

    # Build Eqn 4:
    def build_Fmi(self, Fsp, t1_sp, ttau):
        Fmi = self.get_F(Fsp, 'oo').copy()
        Fmi += ndot('ie,me->mi', t1_sp, self.get_F(Fsp, 'ov'), prefactor=0.5)
        Fmi += ndot('ne,mnie->mi', t1_sp, self.get_MO_sp('ooov'), prefactor=2.0)
        Fmi += ndot('ne,mnei->mi', t1_sp, self.get_MO_sp('oovo'), prefactor=-1.0)
        Fmi += ndot('inef,mnef->mi', np.float32(ttau), self.get_MO_sp('oovv'), prefactor=2.0)
        Fmi += ndot('inef,mnfe->mi', np.float32(ttau), self.get_MO_sp('oovv'), prefactor=-1.0)
        return Fmi

        # Build Eqn 5:

    def build_Fme(self, Fsp, t1_sp):
        Fme = self.get_F(Fsp, 'ov').copy()
        Fme += ndot('nf,mnef->me', t1_sp, self.get_MO_sp('oovv'), prefactor=2.0)
        Fme += ndot('nf,mnfe->me', t1_sp, self.get_MO_sp('oovv'), prefactor=-1.0)
        return Fme

    # Build Eqn 6:
    def build_Wmnij(self, t1_sp, tau):
        Wmnij = self.get_MO_sp('oooo').copy()
        Wmnij += ndot('je,mnie->mnij', t1_sp, self.get_MO_sp('ooov'))
        Wmnij += ndot('ie,mnej->mnij', t1_sp, self.get_MO_sp('oovo'))
        ...
        Wmnij += ndot('ijef,mnef->mnij', np.float32(tau), self.get_MO_sp('oovv'), prefactor=1.0)
        return Wmnij

    # Build Eqn 8:
    def build_Wmbej(self, t1_sp, t2_sp):
        Wmbej = self.get_MO_sp('ovvo').copy()
        Wmbej += ndot('jf,mbef->mbej', t1_sp, self.get_MO_sp('ovvv'))
        Wmbej -= ndot('nb,mnej->mbej', t1_sp, self.get_MO_sp('oovo'))
        tmp = (0.5 * t2_sp)
        tmp += np.einsum('jf,nb->jnfb', t1_sp, t1_sp)
        Wmbej -= ndot('jnfb,mnef->mbej', tmp, self.get_MO_sp('oovv'))
        Wmbej += ndot('njfb,mnef->mbej', t2_sp, self.get_MO_sp('oovv'), prefactor=1.0)
        Wmbej += ndot('njfb,mnfe->mbej', t2_sp, self.get_MO_sp('oovv'), prefactor=-0.5)
        return Wmbej

    # This intermediate appaears in the spin factorization of Wmbej terms.
    def build_Wmbje(self, t1_sp, t2_sp):
        Wmbje = -1.0 * (self.get_MO_sp('ovov').copy())
        Wmbje -= ndot('jf,mbfe->mbje', t1_sp, self.get_MO_sp('ovvv'))
        Wmbje += ndot('nb,mnje->mbje', t1_sp, self.get_MO_sp('ooov'))
        tmp = (0.5 * t2_sp)
        tmp += np.einsum('jf,nb->jnfb', t1_sp, t1_sp)
    ...
    def build_Zmbij(self, tau):
        Zmbij = 0
        Zmbij += ndot('mbef,ijef->mbij', self.get_MO_sp('ovvv'), np.float32(tau))
        return Zmbij
    ...
    def r_T1(self, F, Fsp, t1_sp, t2_sp, ttau):
        ...
        Fae = self.build_Fae(Fsp, t1_sp, ttau)
        Fme = self.build_Fme(Fsp, t1_sp)
        Fmi = self.build_Fmi(Fsp, t1_sp, ttau)
    ...
    def r_T2(self, F, Fsp, t1, t2, t1_sp, t2_sp, tau, ttau):
        ...
        # Build TEI Intermediates in single-precision
        tmp_tau_sp = np.float32(self.build_tau(t1, t2))
        Wmnij = self.build_Wmnij(t1_sp, tau)
        Wmbej = self.build_Wmbej(t1_sp, t2_sp)
        Wmbje = self.build_Wmbje(t1_sp, t2_sp)
        Zmbij = self.build_Zmbij(tau)
