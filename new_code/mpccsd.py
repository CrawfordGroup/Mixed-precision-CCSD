...
from ccpert_sp import CCPert_sp, CCLinresp_sp
...
#precision (0 for dp, 1 for mp, 2 for sp)
precision = 2
print_amps = False
...
for i in range(0, 3):
    ...
    if precision == 0:
        ...
        ccpert[string] = CCPert_sp(string, Mu[string], ccsd, cclambda, omega)
    else:
        print("Please check the input of precision!")
    ...
    ccpert[string].solve('right', r_conv=1e-6)

    print('\nsolving left hand perturbed amplitudes for %s\n' % string)
    ccpert[string].solve('left', r_conv=1e-6)
...
for a in range(0, 3):
    ...
    for b in range(0, 3):
        ...
        if precision == 0:
            ...
            polar_AB[3 * a + b] = CCLinresp_sp(cclambda, ccpert[str_a], ccpert[str_b]).linresp_dp()
        else:
            print("Please check the input of precision!")
