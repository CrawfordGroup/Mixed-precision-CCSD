...
for CCSD_iter in range(1, maxiter + 1):
    ...
    tmp1 = np.einsum('ie,ae->ia', t1, Fae)
    tmp2 = np.einsum('ma,mi->ia', t1, Fmi)
    tmp3 = np.einsum('imae,me->ia', t2, Fme)
    tmp4 = np.einsum('nf,naif->ia', t1, MO[o, v, o, v])
    tmp5 = np.einsum('imef,maef->ia', t2, MO[o, v, v, v])
    tmp6 = np.einsum('mnae,nmei->ia', t2, MO[o, o, v, o])
    ...
    tmp21 = np.einsum('mb,me->be', t1, Fme)
    ...
    tmp22 = np.einsum('je,me->mj', t1, Fme)
    ...
    tmp_tau = build_tau(t1, t2)
    tmp23 = np.einsum('mnab,mnij->ijab', tmp_tau, Wmnij)
    tmp23 = np.float64(tmp23)
    rhs_T2 += 0.5 * tmp23
    tmp24 = np.einsum('ijef,abef->ijab', tmp_tau, Wabef)
    ...
    Pij = np.float64(np.einsum('ie,abej->ijab', t1, MO[v, v, v, o]))
    rhs_T2 += Pij
    rhs_T2 -= Pij.swapaxes(0, 1)

    Pab = np.float64(np.einsum('ma,mbij->ijab', t1, MO[o, v, o, o]))
    rhs_T2 -= Pab
    rhs_T2 += Pab.swapaxes(2, 3)
    ...
    CCSDcorr_E = np.float64(np.einsum('ia,ia->', F[o, v], t1))
    tmpE = np.einsum('ijab,ijab->', MO[o, o, v, v], t2)
    CCSDcorr_E += 0.25 * np.float64(tmpE)
    tmpE = np.einsum('ijab,ia,jb->', MO[o, o, v, v], t1, t1)
    CCSDcorr_E += 0.5 * np.float64(tmpE)
